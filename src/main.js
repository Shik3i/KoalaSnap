  ...
  return `
    <div id="templates-section" class="rounded-2xl border border-white/[6%] bg-[#1a1714] p-4 flex flex-col gap-3">
      ...
      <div id="template-list" class="flex flex-col gap-1 max-h-[140px] overflow-y-auto">
        ${
          templates.length === 0
            ? `<span class="text-[10px] text-zinc-600 text-center py-2">${t('templates.empty')}</span>`
            : templates
                .map(
                  (name) => `
...
const MSG_STATUS = [
  {
    id: 'read',
    title: 'Read',
    svg: `<svg width="13" height="13" viewBox="0 0 16 11" fill="none"><path d="M1 5.5L4.5 9L11 1" stroke="#53bdeb" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/><path d="M6 5.5L9.5 9L16 1" stroke="#53bdeb" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" opacity="0.7"/></svg>`,
  },
  {
    id: 'delivered',
    title: 'Delivered',
    svg: `<svg width="13" height="13" viewBox="0 0 16 11" fill="none"><path d="M1 5.5L4.5 9L11 1" stroke="#8696a0" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/><path d="M6 5.5L9.5 9L16 1" stroke="#8696a0" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" opacity="0.7"/></svg>`,
  },
  {
    id: 'sent',
    title: 'Sent',
    svg: `<svg width="13" height="13" viewBox="0 0 16 11" fill="none"><path d="M1 5.5L4.5 9L11 1" stroke="#8696a0" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>`,
  },
  {
    id: 'unread',
    title: 'Unread',
    svg: `<svg width="8" height="8" viewBox="0 0 8 8" fill="#53bdeb"><circle cx="4" cy="4" r="4"/></svg>`,
  },
];
...
function renderMessageRow(msg, idx) {
  ...
  const groupSenderField =
    state.isGroup && !isSent
      ? `
...
function bindMessageEvents() {
  ...
  list.addEventListener('click', (e) => {
    const btn = e.target.closest(
      '[data-msg-type], [data-msg-status], [data-msg-action], [data-msg-reaction], [data-msg-reaction-clear], [data-msg-image-clear]',
    );
    if (!btn) return;
...
function syncMockup(key, value, state) {
  ...
  try {
    ...
    if (key === 'avatar') {
      const avatarId =
        currentTheme === 'discord' ? 'discord-avatar' : currentTheme === 'whatsapp' ? 'wa-avatar' : 'mockup-avatar';
...
async function copyToClipboard() {
  ...
  try {
    ...
    el.style.transformOrigin = origOrigin;
    bgEls.forEach((bgEl, i) => {
      bgEl.style.backgroundImage = savedBg[i];
    });
    if (blob) {
...
function showToast(msg) {
  ...
  toast.id = 'toast';
  toast.className =
    'fixed bottom-28 left-1/2 -translate-x-1/2 z-50 rounded-full bg-zinc-800/90 backdrop-blur-xl border border-white/10 px-5 py-2.5 text-sm text-zinc-200 shadow-2xl shadow-black/30 animate-fade-in';
...
function getRandomDummySet() {
  ...
  const messages =
    set.messages && set.messages.length > 0
